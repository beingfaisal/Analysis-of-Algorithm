...
void hybridMerge(int* arr, int start, int end)
{
	void merge(int* arr, int start, int mid, int end);
	if ((end - start) > 42)
	{
		int mid = floor((start + end) / 2);
		hybridMerge(arr, start, mid);
		hybridMerge(arr, mid + 1, end);
...
{
	ofstream outFile;
	outFile.open("SortedHybridMergeSort.csv");
	for (int i = 0; i < size; i++)
	{
		outFile << arr[i] << endl;
